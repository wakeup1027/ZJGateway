...
<br clear="both">
	...
	<div style="width: 80%;
    			...
    			color: #fff;">您当前位置：${modular}</div>
		<div class="contentTxt" style="margin-top: 5px;width:80.5%;height: 400px;margin-left: 10%;margin-bottom: 34px;float: left;border: 1px solid #e9e9e9;">
			<ul>
				<s:iterator value="#informationInput">
					<li style="text-indent: 20px;background: url(${pageContext.request.contextPath}/route/images/hsjt.png) no-repeat 10px 10px;line-height: 35px;
						   ...
						   white-space: nowrap;
						   padding-right: 109px;">
						<a href="gateway_detailPage.action?id=${id}" target="_blank" style="color: #666;">${title }</a>
						<span style="float: right;margin-right: 5px;position: absolute;right: 0;
								    font-size: 13px;">[<fmt:formatDate  value="${releaseTime }" type="both" pattern="yyyy-MM-dd" />]
						</span>
					</li>
				</s:iterator>
			...
			<br clear="both">
			<div style="margin-left: 53%;position: absolute; top: 575px;" id="page">第${page }页，共${totalPage }页，${totalNum }条记录&nbsp;&nbsp;跳转到<input id="currentPage" type="text" name="page" style="width:30px" value="${page }" /><label><a id="go" style="color:#00E; cursor: pointer;">&nbsp;GO</a></label>
              <label class="" id="pre">
              <a href="gateway_pagingQuery.action?page=${page-1 }&rows1=${rows1}&modular1=${modular}"><font>上一页</font></a></label>
              <label class="">
              <a href="gateway_pagingQuery.action?page=${page+1 }&rows1=${rows1}&modular1=${modular}"><font>下一页</font></a></label>
          </div>
		</div>
...
<script type="text/javascript">
	$("#go").click(function(){
		window.location.href="gateway_pagingQuery.action?page="+$("#currentPage").val()+"&rows1="+$("#rows1").val()+"&modular1="+$("#modular").val();
	});
	$("#rows1").val("${rows1}");
	
	$("#rows1").change(function(){
		window.location.href="gateway_pagingQuery.action?page="+$("#currentPage").val()+"&rows1="+$(this).val()+"&modular1="+$("#modular").val();
	});
