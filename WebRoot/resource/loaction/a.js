/* 省市区县选择 -end*/
	function promptinfo() {//这个函数是必须的，因为在area.js里每次更改地址时会调用此函数
		...
		with (ss) {
			...
			if (v && document.getElementById(s[v - 1]).selectedIndex > 0 || !v) {
				...
				if (dsy.Exists(str)) {
					...
					if (v)
						options[0].selected = true;// 网上有些版本写的是options[1]，这样的话选择省后，二级市会自动被关联选中，不用鼠标选择，这样的操作不便取值
				}
			}
			if (++v < s.length) {
				change(v);
	...
	// 下面这段代码主要是给默认值，然后页面中body中onload="setup();preselect('陕西省');promptinfo();"
	function preselect(p_key) { //alert(p_key); var index;

		var provinces = new Array("北京市", "天津市", "上海市", "重庆市", "河北省", "山西省",
				"内蒙古", "辽宁省", "吉林省", "黑龙江省", "江苏省", "浙江省", "安徽省", "福建省", "江西省",
				"山东省", "河南省", "湖北省", "湖南省", "广东省", "广西", "海南省", "四川省", "贵州省",
				"云南省", "西藏", "陕西省", "甘肃省", "青海省", "宁夏", "新疆", "香港", "澳门", "台湾省");
		var cnt = provinces.length; // alert(cnt);
		for (i = 0; i < cnt; i++) {
	...
	dsy.add("0", [ "北京市", "天津市", "上海市", "重庆市", "河北省", "山西省", "内蒙古", "辽宁省",
			...
			"甘肃省", "青海省", "宁夏", "新疆", "香港", "澳门", "台湾" ]);

	var s = [ "s1", "s2", "s3" ];
	//var opt0 = ["省份", "地级市", "市、县级市、县"];
	var opt0 = [ "请选择省", "请选择市", "请选择区县" ];//初始化默认值
